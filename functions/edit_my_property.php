...
$sql_check = "SELECT property_name, property_location, property_description FROM properties WHERE property_id = ?";
$stmt_check = $mysqli->prepare($sql_check);
$stmt_check->bind_param("i", $property_id);
$stmt_check->execute();
$stmt_check->bind_result($current_name, $current_location, $current_description);
$stmt_check->fetch();
$stmt_check->close();

// Check if any values have changed
if ($property_name !== $current_name || $property_location !== $current_location || $property_description !== $current_description) {
    // SQL query
    $sql = "UPDATE properties SET property_name = ?, property_location = ?, property_description = ? WHERE property_id = ?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("sssi", $property_name, $property_location, $property_description, $property_id);
