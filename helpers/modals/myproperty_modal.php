<?php
$res = $mysqli->query("SELECT p.property_id, p.property_name, p.property_location, p.property_description, p.property_manager_id, u.user_name AS manager_name FROM properties p INNER JOIN users u ON p.property_manager_id = u.user_id WHERE p.property_manager_id = '{$_SESSION['user_id']}'");
